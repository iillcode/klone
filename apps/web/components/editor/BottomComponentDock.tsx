...
export function BottomComponentDock({
  ...
  return (
    ...
    >
      ...
      {open && hasComponents && (
        <div className="absolute bottom-[calc(100%+14px)] left-1/2 -translate-x-1/2 w-[560px] max-w-[92vw] bg-[#262626] border border-[#3a3a3a] rounded-xl shadow-[0_16px_48px_rgba(0,0,0,0.55)] overflow-hidden flex flex-col">
          {/* Search */}
          <div className="relative p-3.5 pb-2.5">
            <span className="absolute left-6 top-[30px] text-[#9a9a9a] pointer-events-none">
            ...
            <input
              ...
              spellCheck={false}
              autoComplete="off"
              className="w-full h-10 bg-[#1c1c1c] border border-[#3a3a3a] rounded-lg text-[#eee] text-[13px] font-sans px-3 pl-9 outline-none caret-[#0d99f2] placeholder:text-[#8a8a8a] focus:border-[#0d99f2]"
            />
          </div>
                ...
                <button
                  ...
                  className={`border text-[12px] font-medium px-2.5 py-1.5 rounded-md transition-colors ${
                    activeGroups.includes(g.name)
                      ? "bg-[#3d3d3d] text-white border-transparent"
                      : "bg-transparent text-[#9a9a9a] border-transparent hover:text-[#ddd]"
                  }`}
                >
                  {g.name}
                </button>
          ...
          {/* Grouped list */}
          <div className="overflow-y-auto max-h-[min(420px,52vh)] px-3.5 pb-3 pt-1 border-t border-[#333]">
            {visibleGroups.length === 0 ? (
              <div className="py-7 text-center text-[#7a7a7a] text-[12.5px]">
                No components match “{query}”
              </div>
            ) : (
              visibleGroups.map((group) => (
                <div key={group.name} className="mb-1">
                  <h4 className="text-[11px] font-medium text-[#8a8a8a] my-2.5">
                    {group.name}
                  </h4>
                  <div className="grid grid-cols-2 gap-1.5">
                    {group.components.map((component) => (
                      <button
                        ...
                        className="flex items-center gap-2.5 p-2 rounded-lg text-left hover:bg-[#333] transition-colors"
                      >
                        <span className="w-5 h-5 flex-none grid place-items-center text-[#cfcfcf]">
                          <ComponentsIcon className="w-4 h-4" />
                        </span>
                        <span className="flex-1 text-[13px] text-[#ececec] truncate">
                          {component.name}
                        </span>
                        <span className="w-3.5 h-3.5 flex-none grid place-items-center text-[#8a8a8a]">
      ...
      {/* Toolbar */}
      <div className="flex items-center gap-1 bg-[#2c2c2c] border border-[#3a3a3a] rounded-xl p-1.5 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
        <button
          onClick={() => setOpen((o) => !o)}
          ...
          className={`flex items-center gap-1.5 h-10 px-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
            open
              ? "bg-[#0d99f2] text-white"
              : "text-[#e6e6e6] hover:bg-[#3a3a3a]"
          }`}
        >
        ...
        <span className="w-px h-6 bg-[#3a3a3a] mx-0.5" />

        <button
          onClick={onAddPage}
          title="Add page"
          className="flex items-center gap-1.5 h-10 px-3 rounded-lg text-[#e6e6e6] hover:bg-[#3a3a3a] transition-colors"
        >
